...
if($doc_id!="")
{
$sql = "SELECT * FROM appbooking where status !='verified' and   doc_id ='".$doc_id."' order by sno LIMIT $start_from, $limit";
$count_sql = "SELECT count(*) FROM appbooking where status !='verified' and   doc_id ='".$doc_id."'";
}
else
{
$sql = "SELECT * FROM appbooking where  status !='verified' order by sno LIMIT $start_from, $limit";
$count_sql = "SELECT count(*) FROM appbooking where  status !='verified'";
}
...
if (!isset($places)) {
    $places_json = array('currentpage' => $page, 'nextpage' => $next_page, "places" => null, "total_pages" => $total_pages,"total_records"=>$total_records,'doc_id'=>$doc_id);
} else {
    $places_json = array('currentpage' => $page, 'nextpage' => $next_page, "places" => $places, "total_pages" => $total_pages,"total_records"=>$total_records,'doc_id'=>$doc_id);
}
